...
class SpellingTestApp:
    ...
    def setup_gui(self):
        ...
        self.user_id.grid(row=4, column=0, padx=5, pady=5)
        self.user_id.set('Select User')  # Set a placeholder or instruction text

        # Start test button
        ttk.Button(self.test_options_frame, text="Start Test", command=self.start_test).grid(row=5, column=0, padx=5, pady=5)
    ...
    def check_spelling(self):
        ...
        if user_word.strip().lower() == correct_word.strip().lower():
            ...
            if self.current_word in self.words_for_test:
                self.words_for_test.remove(self.current_word)
            ...
            pd.DataFrame({'user_id': [user_id], 'word': [correct_word]}).to_csv(filename, mode='a', header=False, index=False)
